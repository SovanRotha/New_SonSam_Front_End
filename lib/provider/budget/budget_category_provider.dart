...
class BudgetCategoryProvider extends ChangeNotifier{
  ...
  Future<void> getBudgetCategory(int budgetId) async {
    ...
    notifyListeners();
    try{
      final response = await budgetCategoryService.getBudgetCategories(budgetId);
  ...
  Future<void> createBudgetCategory(Map<String, dynamic> budgetCategoryData) async {
     isLoading = true;
    errorMessage = null;
    notifyListeners();
    try{
      final response = await budgetCategoryService.createBudgetCategory(budgetCategoryData);
  ...
  Future<void> updateBudgetCategory(int id, Map<String, dynamic> budgetCategoryData) async {
     isLoading = true;
    errorMessage = null;
    notifyListeners();
    try{
      final response = await budgetCategoryService.updateBudgetCategory(id, budgetCategoryData);

      if (response['budget_categories'] != null) {
        final index = budgetCategories.indexWhere((category) => category.id == id);
        if (index != -1) {
          budgetCategories[index] = BudgetCategory.fromJson(response['budget_categories']);
        }
      }
    }catch (e) {
      // Handle error
