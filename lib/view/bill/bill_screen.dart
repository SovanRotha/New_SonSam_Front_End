  ...
  Future<void> createBill() async {
    ...
    if (created) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(billProvider.errorMessage!)),
      );
    }
  }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          BillDetail(label: 'Amount', value: '\$${bill.amount.toStringAsFixed(2)}'),
                          BillDetail(
                            label: 'Account',
                          ...
                          ElevatedButton.icon(onPressed: (){
                            // Call the deleteBill method from the provider
                            final billProvider = context.read<BillProvider>(); 
                            context.read<BillProvider>().deleteBill(bill.id).then((_) {
                              // Refresh the list of bills after deletion
                              billProvider.getBills();
                            });
